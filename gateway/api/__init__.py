...
class VisioGatewayApi(AIOHTTPService):
    """Control Gateway API."""
    ...
    def __init__(self, gateway,  # config: dict,
        ...
        # self.app = self.create_application(gateway=self._gateway)
    ...
    #     asyncio.run(self.start(app=self.app,
    #                            host=self._host,
    #                            port=self._port
    #                            )
    #                 )
...
if __name__ == '__main__':
    api = VisioGatewayApi(gateway=None, host='localhost', port=7070
                          )

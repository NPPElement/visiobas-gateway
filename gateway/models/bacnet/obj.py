...
from pydantic import BaseModel, Field, Json, PrivateAttr, validator
...
class BACnetObj(BaseBACnetObjModel):
    ...
    segmentation_supported: bool = Field(
        default=False, alias=ObjProperty.segmentationSupported.id_str
    )
    ...
    def validate_resolution(
        cls, v: Optional[Union[int, float]], values
    ...
    def set_pv(self, value: Optional[Union[int, float, str]]) -> None:
        ...
        def _round(value_: Union[float, int], resolution: Union[int, float]) -> float:
    ...
    def _convert_pa_to_str(pa: tuple) -> str:
        ...
        """
        return ",".join(["" if priority is None else str(priority) for priority in pa])

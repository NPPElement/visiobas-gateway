...
from gateway.connectors.bacnet import BACnetObject, ObjProperty, ObjType
...
_log = get_file_logger(
    logger_name=__name__, size_bytes=50_000_000, file_path=_log_file_path
)
...
class BACnetConnector(Thread, Connector):
    ...
    def run(self):
        ...
        while not self.__stopped:
            ...
            if len(self.__polling_devices) > 0:
                ...
                irrelevant_devices_id = tuple(
                    set(self.__polling_devices.keys()) - set(self.device_ids)
                )
    ...
    def update_devices(
        self, devices: dict[int, set[BACnetObject]], update_intervals: dict[int, int]
    ) -> None:
    ...
    def __start_device(
        self, device_id: int, objects: set[BACnetObject], update_interval: int
    ) -> None:
        ...
        try:
            ...
            _log.error(f"Device [{device_id}] " f"starting error: {e}", exc_info=True)
        else:
    ...
    def __stop_device(self, device_id: int) -> None:
        ...
        try:
            ...
            _log.error(
                f"The device with id {device_id} is not running. "
                f"Please provide the id of the polling device: {e}"
            )
        except Exception as e:
            _log.error(f"Device stopping error: {e}")

    def get_devices_objects(
        self, device_ids: Iterable[int], obj_types: tuple
    ...
    def get_devices_update_interval(
        self, devices_id: Iterable[int], default_update_interval: int
    ) -> dict[int, int]:

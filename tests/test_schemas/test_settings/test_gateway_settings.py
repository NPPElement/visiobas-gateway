...
class TestGatewaySettings:
    @pytest.mark.parametrize(
        ...
        [
            (
                    {
                        'update_period': 4444, 'unreachable_reset_period': 999,
                        'unreachable_threshold': 2, 'override_threshold': 10,
                        'poll_device_ids': [11, 22]
                    }, 4444, 999, 2, Priority.GCL_PLUS, [11, 22],
            ),
            (
                    {'poll_device_ids': [12, 13]},
                    3600, 1800, 3, Priority.MANUAL_OPERATOR, [12, 13]
            ),
        ],
    )
    def test_construct_happy(
        ...
        assert gateway_settings.unreachable_reset_period == expected_unreachable_reset_period
